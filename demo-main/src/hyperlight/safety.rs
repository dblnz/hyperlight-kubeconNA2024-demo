...
pub(crate) fn deref_raw_null_ptr(
        ...
        .and_then(move || async {
            ...
            match result {
                Ok(_) => Ok::<_, warp::Rejection>(
                    "Guest function called inside warm Hyperlight VM.".to_string(),
                ),
...
pub(crate) fn stack_overflow(
    ...
    warp::path("hyperlight")
        ...
        .and_then(move || async {
            ...
            let result = {
                let mut sandbox_guard = sandbox.lock().await;
                sandbox_guard.call_guest_function_by_name("StackOverflow", ReturnType::Int, Some(vec![ParameterValue::Int(100)]))
            };
            // `sandbox_guard` goes out of scope here and is dropped
